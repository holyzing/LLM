...
def mt_add(itemId, shopId, sessionId, userId, token, Device_ID):
    ...
    response = requests.post(
        'https://app.moutai519.com.cn/xhr/front/mall/reservation/add', headers=headers, json=json_data
    )
...
def tongzhi(ss):
    ...
    for user in user_list:
        url = 'http://wxpusher.zjiecode.com/api/send/message/?appToken=&content={}&uid={}'.format(ss, user)
        r = requests.get(url)
        print(r.text)
...
def get_shop_item(sessionId, itemId, device_id, token, province, city):
    ...
    base_url = 'https://static.moutai519.com.cn/mt-backend/xhr/front/mall/shop/list/slim/v3/'
    response = requests.get(f'{base_url}{sessionId}/{province}/{itemId}/{time_keys}', headers=headers)
...
def getUserEnergyAward(device_id, ck):
    ...
    response = requests.post(
        'https://h5.moutai519.com.cn/game/isolationPage/getUserEnergyAward', cookies=cookies, headers=headers, json={}
    )
...
def login(phone, vCode, Device_ID):
    ...
    response = requests.post(
        'https://app.moutai519.com.cn/xhr/front/user/register/login', headers=headers, json=json_data
    )
    data = response.json().get('data', {})
    ...
    if len(mt_token_list) > 0:
        for mt_token in mt_token_list:
            ...
            try:
                ...
                province, city, lng, lat, device_id, token, ck = (
                    province.strip(),
                    city.strip(),
                    lng.strip(),
                    lat.strip(),
                ...
                logging.error(f"MTTokenD格式错误: {mt_token}, 错误信息: {e}")
            ...
            try:
                ...
                if not user_id:
                    ...
                    continue
                s += f"第{userCount}个用户----------------{userName}_{mobile}开始任务----------------\n"
